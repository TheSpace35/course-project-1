...
settingsBtn.addEventListener('click', e=>{
    ...
    if(mode !== 'settings'){
        ...
        const adminData = getTable('ADMINS').find(admin => admin.id === JSON.parse(localStorage.getItem('session')).id);


        const form = document.querySelector('.settings__layout__form');
        form.login.value = adminData.Login;
        form.email.value = adminData.Email;
        form.phone.value = adminData.Phone;


        /// Изменение данных администратора ///

        document.querySelector('.settings__layout__form_saveBtn_admin').addEventListener('click', e=>{
            e.preventDefault();
            ...
            console.log(validate('ChangeAdminData', { "type": "login", "element": form.login },{ "type": "email", "element": form.email },{ "type": "phone", "element": form.password }, { "type": "passwordConfirm", "element": form.passwordConfirm}));


            ChangeUserData('admin', JSON.parse(localStorage.getItem('session')).id, {"Login":`${form.login.value}`,"Password":`${form.password.value}`, "Email":`${form.email.value}`, "Phone":`${form.phone.value}`});
    ...
    clients.forEach(el=>{
        ...
        `<td colspan="7">
            <table>
                ...
                <tr>
                    <td><button class='clients__table_addProjectBtn account-btn'>Добавить проект</button></td>
                </tr>
            </table>
    ...
    clientAddTable.querySelectorAll('.clients__table_edit').forEach(el => {
        ...
        el.addEventListener('click', e => {
            ...
            if (currentClient) {
                ...
                const form = clientEditPopup.querySelector('.clientEdit-form');
                form.Company.value = currentClient.Company;
                form.Phone.value = currentClient.Phone;
                form.Login.value = currentClient.Login;
                form.Email.value = currentClient.Email;
                ...
                clientEditFormBtn.addEventListener('click', e => {
                    ...
                    const validCheck = validate("editClient", { "type": "company", "element": form.company }, { "type": "phone", "element": form.phone }, { "type": "email", "element": form.email }, { "type": "login", "element": form.login });
                    if (!validCheck.length > 0) {
                        ...
                        validCheck.forEach(el => {
                            ...
                            form.querySelectorAll('.clientEdit-form__input').forEach(e => {
                                if (e.name === el.type) {
                                    e.nextElementSibling.style.opacity = 1;
...
function initApplications(){
    ...
    applicationsTable.querySelectorAll('.applications__table_CompleteBtn').forEach(el=>{
        ...
        el.addEventListener('click', e=>{
            ...
            let applications = getTable('APPLICATIONS');
            const currentApp = applications.find(app => app.id === el.closest('tr').querySelector('.applications__table_id').textContent);
            console.log(currentApp);
            
            if (currentApp) {
                console.log('click2');
                currentApp.Status = 'Обработано';
                localStorage.setItem('APPLICATIONS', JSON.stringify(applications));

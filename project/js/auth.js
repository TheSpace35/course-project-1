...
function Login(){
    ...
    for(admin of JSON.parse(localStorage.getItem('ADMINS'))){
        if(dataLogin === admin.Login && dataPassword === admin.Password){
            ...
            setTimeout(() => {
                if(dataSave){
                    localStorage.setItem('session', JSON.stringify({'role':"admin","id":`${admin.id}`,'save':true})); 
                }
                else{
                    localStorage.setItem('session', JSON.stringify({'role':"admin", "id":`${admin.id}`})); 
                }
    ...
    for(client of JSON.parse(localStorage.getItem('CLIENTS'))){
        if(dataLogin === client.Login && dataPassword === client.Password){
            ...
            setTimeout(() => { 
                if(dataSave){
                    localStorage.setItem('session', JSON.stringify({'role':"client","id":`${client.Id}`,'save':true})); 
                }

                else{
                    localStorage.setItem('session', JSON.stringify({'role':"client", "id":`${client.Id}`})); 
                }
